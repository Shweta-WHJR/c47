...
function preload(){
  ...
  aim = loadImage("sprites/Pool-Stick.png")
}
...
function draw() {
  ...
  if(keyIsDown(DOWN_ARROW) && aimPower > 40){
    aimPower--;
  }
  ...
  if(showAim === true){
    ...
    push();
    translate(striker.body.position.x + (-50 + ((aimPower-10)*-0.4)) + aimPower/2.5, striker.body.position.y);
    rotate(aimAngle/200);
    image(aim, 0-50, 0, 50 + aimPower , 150);
    pop();
